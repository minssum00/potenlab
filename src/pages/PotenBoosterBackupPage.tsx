...
export function PotenBoosterBackupPage() {
  ...
  const content = {
    ko: {
      ...
      mvpSample: {
        label: "MVP Sample",
        title: "실제 구현 사례를 확인하세요",
        description: "다양한 비즈니스 유형별로 어떻게 MVP를 구현하고 검증했는지 살펴보세요.",
        cards: [
          {
            category: "매칭 플랫폼형",
            title: "1인 가구 전문 청소 매칭 서비스 '클린메이트'",
            coreLogic: "단순 매칭 (유저 신청 → 매니저 배정)",
            proofPoint: "복잡한 앱 없이도 실제 예약 DB가 일어나는 핵심 루프 구현.",
            quote: "막연한 매칭 플랫폼 아이디어, 3일 만에 실제 시장 데이터가 발생하는 서비스",
          },
          {
            category: "자동화/AI형",
            title: "스마트 영양제 맞춤 추천 서비스 '핏타민'",
            coreLogic: "퀴즈 기반 추천 (설문 데이터 → 결과 도출)",
            proofPoint: "POSTECH 석사 기반의 R&D 전문성을 살려 사용자 맞춤형 로직이 웹에서 구현되는 과정 노출.",
            quote: "어려운 추천 알고리즘, 복잡한 개발 없이도 즉시 시장의 반응을 확인",
          },
          {
            category: "B2B/유틸리티형",
            title: "소상공인용 AI 법률 상담 챗봇 '로봇 변호사'",
            coreLogic: "API 연동 (GPT/LLM 기반 상담 기능 제공)",
            proofPoint: "최신 AI 기술을 활용해 작동하는 MVP를 얼마나 빠르게 배포할 수 있는지 시각화.",
            quote: "단순한 챗봇이 아닙니다. 비즈니스 가치를 지닌 AI 서비스를 가장 빠르게 세상에 내놓으세요.",
          },
          {
            category: "커뮤니티/콘텐츠형",
            title: "취향 기반 원데이 클래스 모임 '모여라'",
            coreLogic: "데이터 수집 및 관리 (모임 생성 → 유저 참여 데이터 분석)",
            proofPoint: "메타 광고 지표(CTR)와 연동하여 유저가 실제로 모이는지 데이터로 확인하는 과정.",
            quote: "추측성 근거 대신, 3일간의 광고 집행으로 얻은 진짜 유저 지표를 확보하세요.",
          },
        ],
      },
      whyUs: {
    ...
    en: {
      ...
      mvpSample: {
        label: "MVP Sample",
        title: "See Real Implementation Cases",
        description: "Explore how we built and validated MVPs across different business types.",
        cards: [
          {
            category: "Matching Platform",
            title: "Solo Living Cleaning Matching Service 'CleanMate'",
            coreLogic: "Simple Matching (User Request → Manager Assignment)",
            proofPoint: "Implementing the core booking loop that generates actual reservation data without a complex app.",
            quote: "Turn a vague matching platform idea into a service generating real market data in just 3 days",
          },
          {
            category: "Automation/AI",
            title: "Smart Supplement Recommendation Service 'FitVitamin'",
            coreLogic: "Quiz-based Recommendation (Survey Data → Result Generation)",
            proofPoint: "Showcasing the process of implementing personalized recommendation logic on the web, leveraging POSTECH master's-level R&D expertise.",
            quote: "Validate complex recommendation algorithms instantly without complicated development",
          },
          {
            category: "B2B/Utility",
            title: "AI Legal Consultation Chatbot for SMBs 'Robot Lawyer'",
            coreLogic: "API Integration (GPT/LLM-based Consultation)",
            proofPoint: "Visualizing how quickly an MVP utilizing the latest AI technology can be deployed.",
            quote: "Not just a simple chatbot. Launch an AI service with real business value faster than ever.",
          },
          {
            category: "Community/Content",
            title: "Interest-based One-day Class Meetup 'Moyeora'",
            coreLogic: "Data Collection & Management (Meetup Creation → User Participation Data Analysis)",
            proofPoint: "The process of verifying whether users actually gather through integration with Meta ad metrics (CTR).",
            quote: "Instead of speculative evidence, secure real user metrics from 3 days of ad campaigns.",
          },
        ],
      },
      whyUs: {
      ...
      <section className="py-16 sm:py-24 lg:py-32 px-4 sm:px-6 lg:px-8 bg-[#F8F9FA]">
        <div className="max-w-[1156px] mx-auto">
          ...
          >
            ...
            <p className="text-[13px] sm:text-[14px] lg:text-[15px] text-[#666666] max-w-[600px] mx-auto px-2">
              {t.mvpSample.description}
            </p>
          </motion.div>

          <div className="grid sm:grid-cols-2 gap-5 lg:gap-6">
            {t.mvpSample.cards.map((card, idx) => (
              <motion.div
                key={idx}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                ...
                onClick={() => navigate("contact-success")}
                className="bg-white p-6 sm:p-7 lg:p-8 rounded-2xl border border-[#E5E7EB] hover:border-[#0079FF]/50 hover:shadow-[0_8px_30px_rgba(0,121,255,0.1)] transition-all duration-300 cursor-pointer group"
              >
                <div className="mb-4">
                  <span className="inline-block text-[#0079FF] px-3 py-1 rounded-full text-[12px] sm:text-[13px] font-medium border border-[#0079FF]/30 bg-[#E6F3FF]">
                    {card.category}
                  </span>
                </div>
                <h3 className="text-[16px] sm:text-[18px] lg:text-[20px] font-bold text-[#0E1116] mb-4 leading-tight">
                  {card.title}
                </h3>
                <div className="space-y-3 mb-5">
                  <div>
                    <p className="text-[12px] sm:text-[13px] font-semibold text-[#0079FF] mb-1">핵심 로직</p>
                    <p className="text-[13px] sm:text-[14px] text-[#666666] leading-relaxed">
                      {card.coreLogic}
                    </p>
                  </div>
                  <div>
                    <p className="text-[12px] sm:text-[13px] font-semibold text-[#0079FF] mb-1">증명 포인트</p>
                    <p className="text-[13px] sm:text-[14px] text-[#666666] leading-relaxed">
                      {card.proofPoint}
                    </p>
                  </div>
                </div>
                <div className="bg-[#F8F9FA] p-4 rounded-xl border-l-4 border-[#0079FF]">
                  <p className="text-[13px] sm:text-[14px] text-[#424242] leading-relaxed italic">
                    "{card.quote}"
                  </p>
                </div>
              </motion.div>
